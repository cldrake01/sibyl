                    ...
                    if causal:
                        ...
                        if striped_ring_attn:
                            ...
                            causal_mask = torch.ones(
                                (qc.shape[-3], kc.shape[-3]),
                                dtype=torch.bool,
                                device=device,
                            ).triu(triu_offset + qk_len_diff)
        ...
        for (ring_rank, is_last), (
            ...
            for k_ind, (kc, vc, dkc, dvc, col_mask) in enumerate(col_splits):
                ...
                for ind, (qc, oc, doc, lsec, dqc) in enumerate(row_splits):
                    ...
                    if exists(col_mask):
                        p = einx.where("b j, b h i j, -> b h i j", col_mask, p, 0.0)

                    dv_chunk = torch.einsum("b h i j, b i h d -> b j h d", p, doc)
                    dp = torch.einsum("b i h d, b j h d -> b h i j", doc, vc)

                    D = (doc * oc).sum(dim=-1, keepdims=True)
                    D = rearrange("b n h 1 -> b h n 1", D)
                    ds = p * scale * (dp - D)

                    dq_chunk = torch.einsum("b h i j, b j h d -> b i h d", ds, kc)
                    dk_chunk = torch.einsum("b h i j, b i h d -> b j h d", ds, qc)
